...
bootentry_add(int type, int prio, u32 data, const char *desc)
{
    ...
    be->data = data;
    be->description = desc ?: "?";
    dprintf(1, "Registering bootable: %s (type:%d prio:%d data:%x)\n"
...
#define DEFAULT_BOOTMENU_WAIT 25000
...
boot_disk(u8 bootdrv, int checksig)
{
    ...
    br.al = 1;
    br.cl = 1;
    printf("DEBUG boot disk %d flags %d\n",bootdrv,br.flags);
    call16_int(0x13, &br);
    printf("After call to boot int 13 br.flags %d bootdrv %d\n",br.flags,bootdrv);
    if (br.flags & F_CF) {
